...
def total_over_bound(arr, over_total, s=0, e=max):
    if s >= e:
        return s

    mid = (s + e) // 2
    if get_over_sum(arr, mid) >= over_total:
        return total_over_bound(arr, over_total, mid + 1, e)
    else:
        return total_over_bound(arr, over_total, s, mid)
...
# over_bound에서 1빼서 조건에 맞는 최대값!
print(total_over_bound(trees, m) - 1)
